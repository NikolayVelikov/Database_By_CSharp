...
namespace P03_FootballBetting.Data
{
    public class FootballBettingContext : DbContext
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            ...
            {
                optionsBuilder.UseSqlServer(Connections.dbConnect);
            }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            {
                x.HasOne(x => x.PrimaryKitColor).WithMany(x => x.PrimaryKitTeams).HasForeignKey(x => x.PrimaryKitColorId).OnDelete(DeleteBehavior.Restrict);
            });

            modelBuilder.Entity<Team>(x =>
            {
                x.HasOne(x => x.SecondaryKitColor).WithMany(x => x.SecondaryKitTeams).HasForeignKey(x => x.SecondaryKitColorId).OnDelete(DeleteBehavior.Restrict);
            });
