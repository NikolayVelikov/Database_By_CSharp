...
namespace CreatingAndWorkingWithDatabase
{
    public class StartUp
    {
        ...
        {
            string sqlConncetionString1 = @"Data Source=DESKTOP-8F63LT6\TEW_SQLEXPRESS;Database=master;Integrated Security=true";
            string baseName = "MinionsDB";
            string dataBaseName = $"CREATE DATABASE {baseName}";
            using (var conncetion = new SqlConnection(sqlConncetionString1))
            {
                conncetion.Open();

                ExecudeQuery(dataBaseName, conncetion); // part 1

                conncetion.Close();
            }

            string sqlConncetionString2 = @"Data Source=DESKTOP-8F63LT6\TEW_SQLEXPRESS;Database=" + baseName + ";Integrated Security=true";
            using (var conncetion = new SqlConnection(sqlConncetionString2))
            {
        ...
        private static void ExecudeQuery(string query, SqlConnection connection)
        {
        ...
        private static string[] TableInformation()
        {
